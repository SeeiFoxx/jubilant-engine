...
def tutoriel_suite(niveau = 0):
	...
	screen.blit(background, (0, 0))
	# Pour un niveau débutant ou apprenti, on mets le premier texte simple à comprendre.
	if (niveau == 0) or (niveau == 1):
		text = "Année 2060. Dans le futur, les gens ont de plus en plus de difficultés avec les monnaies classiques car elles perdent de la valeur rapidement. Pour éviter cela, les gens se tournent vers les cryptomonnaies, qui sont des monnaies numériques sécurisées et anonymes. Les cryptomonnaies étaient autrefois uniquement considérées comme des blagues, mais elles sont maintenant très importantes dans le monde entier, car elles maintiennent leur valeur avec le temps. Une personne mystérieuse appelée EL qui dirige une entreprise d'échange de cryptomonnaies appelée Pycoin Services. On ne sait pas grand-chose à son sujet, mais une rumeur court sur le fait qu'il a développé une IA capable de l'aider dans son travail de gestion de la plateforme, avant de disparaître peu de temps après. Tokyo, 3 heures du matin. Vous êtes réveillé par une notification particulière: votre compte Pycoin a été crédité d'un pass d'accès que vous ne connaissez pas. Ce pass permets d'accéder à la direction de Pycoin Services: vous êtes maintenant le patron secret de plus grande plateforme d'échange de cryptomonnaies en ligne du monde."
	# Pour un niveau confirmé ou expert, on mets le second texte plus compliqué.
	if (niveau == 2) or (niveau == 3):
		text = "2060. Les taux d'inflation des monnaies étatiques (dites fiat) ont fait fuir les populations du monde vers les cryptomonnaies. Ces monnaies digitales, sécurisées et anonymes, qui étaient pour la plupart considérées comme des blagues, deviennent un enjeu majeur au niveau mondial de par leur propriétés déflationnistes par nature. À la tête des fameux Pycoin Services, la plus grade plateforme d'échange anonyme de cryptommonaies, se tient un mystérieux individu: EL. Presque aussi mythique que le grand Satoshi Nakamoto, beaucoup de rumeurs circulent à son sujet. On dit qu'il aurait inventé une intelligence artificielle assez poussée pour l'aider dans la gestion du site, avant d'avoir disparu peu de temps après. Tokyo, 3 heures du matin. Vous êtes réveillé par une notification particulière: votre compte Pycoin vient d'être crédité d'un token inconnu, permettant la gestion des Pycoin Services."
